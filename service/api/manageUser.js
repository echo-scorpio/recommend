...
router.post('/showAdminList', (req, res) => {
    ...
    conn.query(sql, [], function (err, result) {
        var data = JSON.parse(JSON.stringify(result));
        data.forEach(ele => {
            ele.admin_right = ele.admin_right.split(',')
        });
...
router.post('/editRight', (req, res) => {
    ...
    conn.query(delSql, [params.id], function (err, result) {       
            ...
            rights.forEach(element => {
                if (element != '') {
                    conn.query(insertRight, [params.id, element])
                }
            });
            var editAdminName=$sql.manageUser.editAdminName;
            conn.query(editAdminName,[params.name,params.id])
            return res.send({
                msg: 'success'
            })
